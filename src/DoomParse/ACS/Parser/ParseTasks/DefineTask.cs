...
internal readonly struct DefineTask : IParseTask
{
	public bool TryParse(ParseContext context, ACSTokenizer tokenizer, [MaybeNullWhen(true)] out FeatureBase? feature)
	{
		...
		{
			tokenizer.Next();
			if (!tokenizer.Symbol.Equals("define", context.DefaultComparison))
			{
				context.Logger.LogDebug("Hash does not represent define definition.");

				// Reset since it could be `#library`, `#libdefine`, `#import` or `#include`.
				feature = null;
				return false;
			}
		...
		context.Logger.LogDebug("Start parse define definition.");
		tokenizer.Next();
		...
		feature = new DefineFeature(key, value);
		return true;
	}
}
