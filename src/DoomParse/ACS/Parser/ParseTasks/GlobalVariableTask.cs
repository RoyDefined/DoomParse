...
internal readonly struct GlobalVariableTask : IParseTask
{
	public bool TryParse(ParseContext context, ACSTokenizer tokenizer, [MaybeNullWhen(true)] out FeatureBase? feature)
	{
		if (tokenizer.Token != TGLOBALVAR)
		{
			feature = null;
			return false;
		}

		context.Logger.LogDebug("Start parse global variable definition.");
		tokenizer.Next();
		if (tokenizer.Token != TSYMBOL)
		{
			context.Exception = new("Expected global variable parameter type.");
			feature = null;
			return false;
		}
		...
		{
			context.Exception = new("Expected global variable index.");
			feature = null;
			return false;
		}
		...
		{
			context.Exception = new("Expected colon after global variable index.");
			feature = null;
			return false;
		}
		...
		if (tokenizer.Token == TLBRACKET)
		{
			...
			{
				context.Exception = new("Expected closing array brackets after global variable declaration.");
				feature = null;
				return false;
			}
		...
		{
			context.Exception = new("Expected semicolon after global variable declaration.");
			feature = null;
			return false;
		}

		feature = new GlobalVariableFeature(variableType, variableIndex, variableName, isArray);
		return true;
	}
}
